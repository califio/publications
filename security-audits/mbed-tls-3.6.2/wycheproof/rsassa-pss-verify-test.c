...
int test_rsa_verify_pss(json_t *test_group);
...
int load_rsa_key(mbedtls_rsa_context *rsa, json_t *test_group) {
    ...
    size_t len_sig = mbedtls_mpi_size(&N);
    printf("len N = %zu\n",len_sig);
...
int test_rsa_verify_pss(json_t *test_group) {
    ...
    json_array_foreach(tests, index, test_case) {
        ...
        result = mbedtls_rsa_rsassa_pss_verify(&rsa, md_type, md_size, hash, sig);
        ...
        if (strcmp(expected, "valid") == 0) {
            ...
            if (result == 0) {
                ...
                printf("Test case %d: ACCEPTABLE (signature rejected)\n", (int)json_integer_value(json_object_get(test_case, "tcId")));
            }
        } else {
...
int main(int argc, char *argv[]) {
    ...
    json_array_foreach(test_groups, index, test_group) {
        ...
        test_rsa_verify_pss(test_group);
    }
