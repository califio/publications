...
void test_rsa_decrypt(json_t *test_group) {
    ...
    json_array_foreach(tests, index, test_case) {
        ...
        unsigned char ct[BUFFER_SIZE], pt[BUFFER_SIZE], expected_pt[BUFFER_SIZE];
        size_t ct_len, pt_len = 0, expected_len;
        ...
        ret = mbedtls_rsa_pkcs1_decrypt(&rsa, dummy_random, NULL, &pt_len, ct, pt, BUFFER_SIZE);
        if(ret == 0){
